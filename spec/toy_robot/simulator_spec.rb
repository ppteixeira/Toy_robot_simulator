...
describe ToyRobot::Simulator do
  ...
  it "turning an unplaced robot to left does not cause an exception" do
    expect { subject.turn_left }.to_not raise_error
  end

  it "turning an unplaced robot to right does not cause an exception" do
  ...
  it "informe when a command is invalid" do
  ...
  context "when robot has been placed" do
    ...
    it "tells the robot to report" do
      ...
      message = "Robot is currently at (3, 3) and it is facing NORTH\n"
  ...
  context "robot placed at table boundary" do
    ...
    it "cannot move past the table boundary" do
      subject.move
      message = "Robot is currently at (0, 4) and it is facing NORTH\n"
      expect { subject.report }.to output(message).to_stdout
    end
  end
end #describe ToyRobot::Simulator
